...
namespace FTimeHelper
{
    ...
    constexpr uint32 Abs(int32 N) { return uint32(N < 0 ? -N : N); }
...
consteval static TStaticArray<FVector, 8> GenerateCorners()
{
    TStaticArray<FVector3, 8> Corners{{FVector{1}}};
...
bool IsVisible(const FRenderObject& Obj, const FMatrix& ViewProj)
{
    constexpr auto Corners = GenerateCorners();
    FMatrix Matrix = ViewProj * Obj.Transform;
    
    FVector Min{1.5, 1.5, 1.5};
    FVector Max{-1.5, -1.5, -1.5};
    
    for (int32 c = 0; c < Corners.Num(); c++)
    {
        // project each corner into clip space
        FVector4 v = Matrix * FVector4(Obj.Bounds.Origin + (Corners[c] * Obj.Bounds.Extents), 1.f);
...
bool VulkanEngine::Init()
{
    ...
    InitVulkan();
    fmt::println("Init swapchain");
    InitSwapchain();
    fmt::println("Init Commands");
    InitCommands();
    fmt::println("Init Sync");
    InitSyncStructures();
    fmt::println("Init Descriptors");
    InitDescriptors();
    
    fmt::println("Init Pipelines");
    InitPipelines();
    
    fmt::println("Init Defdata");
    InitDefaultData();
        
    fmt::println("Init Renderable");
    InitRenderable();
...
void VulkanEngine::DrawMain(VkCommandBuffer Cmd)
{
    ...
    vkCmdDispatch(Cmd, FMath::CeilToInt<uint32>(DrawExtent.width / 16.f), FMath::CeilToInt<uint32>(DrawExtent.height / 16), 1);
...
void VulkanEngine::DrawGeometry(VkCommandBuffer Cmd)
{
    ...
    TArray<uint32> OpaqueDraws;
    OpaqueDraws.Reserve(MainDrawContext.OpaqueSurfaces.Num());
    
    for (uint32 i = 0; i < MainDrawContext.OpaqueSurfaces.Num(); ++i) {
        if (IsVisible(MainDrawContext.OpaqueSurfaces[i], SceneData.ViewProjection)) {
            OpaqueDraws.Add(i);
        }
    }
    
    TArray<uint32> TransparentDraws;
    TransparentDraws.Reserve(MainDrawContext.TransparentSurfaces.Num());
    
    for (uint32 i = 0; i < MainDrawContext.TransparentSurfaces.Num(); ++i) {
        if (IsVisible(MainDrawContext.TransparentSurfaces[i], SceneData.ViewProjection)) {
            TransparentDraws.Add(i);
        }
    }
    
    //sort opaque surfaces by material and mesh
    //@TODO Actually implement a sort method
    /*std::sort(OpaqueDraws.begin(), OpaqueDraws.end(), [&](uint32 iA, uint32 iB)
    {
        const FRenderObject& A = MainDrawContext.OpaqueSurfaces[iA]; 
        const FRenderObject& B = MainDrawContext.OpaqueSurfaces[iB];
    ...
    *SceneUniformData = SceneData;
    ...
    const FMaterialPipeline* LastPipeline = nullptr;
    const FMaterialInstance* LastMaterial = nullptr;
    VkBuffer LastIndexBuffer = VK_NULL_HANDLE;
    
    auto Draw = [&](const FRenderObject& RenderObject)
    {
        const FMaterialInstance* CurrentMat = RenderObject.MaterialInstance;
        if (CurrentMat != LastMaterial)
        {
            LastMaterial = CurrentMat;
            //rebind pipeline and descriptors if the material changed
            if (CurrentMat->Pipeline != LastPipeline)
            {
                LastPipeline = CurrentMat->Pipeline;
            ...
            vkCmdBindDescriptorSets(Cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, LastPipeline->Layout, 1, 1, 
                &RenderObject.MaterialInstance->MaterialSet, 0, nullptr);
        }
        ...
        if (RenderObject.IndexBuffer != LastIndexBuffer)
        {
            ...
            vkCmdBindIndexBuffer(Cmd, LastIndexBuffer, 0, VK_INDEX_TYPE_UINT32);
        }
        
        FGpuDrawPushConstants Push{};
        Push.WorldMatrix = RenderObject.Transform;
        Push.VertexBuffer = RenderObject.VertexBufferAddress;
        
        vkCmdPushConstants(Cmd, LastPipeline->Layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(Push), &Push);
        vkCmdDrawIndexed(Cmd, RenderObject.IndexCount, 1, RenderObject.FirstIndex, 0, 0);
        
        //add counters for triangles and draws
        Stats.DrawcallCount++;
        Stats.TriangleCount += RenderObject.IndexCount / 3;   
    };
    
    for (uint32 i : OpaqueDraws)
    {
        Draw(MainDrawContext.OpaqueSurfaces[i]);
    }
    
    for (uint32 i : TransparentDraws)
    {
        Draw(MainDrawContext.TransparentSurfaces[i]);
    }
...
void VulkanEngine::UpdateScene()
{
    ...
    for (int32 i = -3; i < 3; ++i)
    {
        FMatrix Scale = FTransform::Scale(FVector{.2f});
...
void FMeshNode::Draw(const FMatrix& TopMatrix, FDrawContext& Ctx)
{
    ...
    for (auto& Surface : Mesh->Surfaces)
    {
        ...
        if (Surface.Material->Data.PassType == EMaterialPass::Opaque)
            Ctx.OpaqueSurfaces.Add(Def);
        else if (Surface.Material->Data.PassType == EMaterialPass::Transparent)
            Ctx.TransparentSurfaces.Add(Def);
    }
...
void FGltfMetalicRoughness::BuildPipelines(VulkanEngine& Engine)
{
    ...
    if (vkutil::LoadShaderModule("Mesh.frag.spv", Engine.Device, MeshFragShader, &FullBuild)) {
        fmt::println("Note Success when building the triangle fragment shader module {}", FullBuild);
    }
    else {
        fmt::println("Error when building the triangle fragment shader module {}", FullBuild);
    }
    
    VkShaderModule MeshVertexShader;
    if (vkutil::LoadShaderModule("Mesh.vert.spv", Engine.Device, MeshVertexShader, &FullBuild)) {
        fmt::println("Note Success when building the triangle vertex shader module {}", FullBuild);
    }
    else {
        fmt::println("Error when building the triangle vertex shader module {}", FullBuild);
    }
...
void VulkanEngine::Draw()
{
    ...
    VkSemaphoreSubmitInfo SignalInfo = Vkinit::semaphore_submit_info(VK_PIPELINE_STAGE_2_ALL_GRAPHICS_BIT, PresentSemaphores[SwapchainImageIndex]);
    ...
    PresentInfo.pWaitSemaphores = &PresentSemaphores[SwapchainImageIndex];
...
{
    const size_t VertexBufferSize = Vertices.Num() * sizeof(FVertex);
    const size_t IndexBufferSize = Indices.Num() * sizeof(uint32);
    ...
    //Copy vertex buffer
    FMemory::Memcpy(Data, Vertices.GetData(), VertexBufferSize);
    
    //Copy index buffer
    FMemory::Memcpy(static_cast<char*>(Data) + VertexBufferSize, Indices.GetData(), IndexBufferSize);
...
void VulkanEngine::InitBackgroundPipelines()
{
    ...
    if (!vkutil::LoadShaderModule("gradient_color.comp.spv", Device, GradientShader, &FullBuild))
    {
        fmt::println("Error when building the compute shader at {}", FullBuild);
        return;
    }
    
    VkShaderModule SkyShader;
    if (!vkutil::LoadShaderModule("sky.comp.spv", Device, SkyShader, &FullBuild))
    {
        fmt::println("Error when building the compute shader at {}", FullBuild);
        return;
    }
...
void VulkanEngine::InitTrianglePipeline()
{
    ...
    if (!vkutil::LoadShaderModule("TexImage.frag.spv", Device, TriangleFragShader, &FullBuild))
    {
        fmt::println("Error when building the fragment shader at {}", FullBuild);
    }
    else
    {
        fmt::println("Triangle fragment shader succesfully loaded at {}", FullBuild);
    }
    VkShaderModule TriangleVertexShader;
    if (!vkutil::LoadShaderModule("ColoredTriangle.vert.spv", Device, TriangleVertexShader, &FullBuild))
    {
        fmt::println("Error when building the vertex shader at {}", FullBuild);
    }
    else
    {
        fmt::println("Triangle vertex shader succesfully loaded at {}", FullBuild);
    }
...
void VulkanEngine::InitDefaultData()
{
    ...
    for (uint32 x = 0; x < ImageSize; ++x) {
        for (uint32 y = 0; y < ImageSize; ++y) {
            Pixels[y * ImageSize + x] = ((x & 1) ^ (y & 1)) ? Magenta : Colors[1];
        }
    }
    
    ErrorCheckerboardImage = CreateImage(Pixels.GetData(), VkExtent3D{ImageSize, ImageSize, 1}, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT);
...
void VulkanEngine::InitDescriptors()
{
    ...
    for (FFrameData& Frame : Frames) {
        ...
        TArray<FDescriptorAllocator::FPoolSizeRatio> FrameSizes{
                { VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 3},
                { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 3},
        ...
        Frame.FrameDescriptors.Init(Device, 1000, FrameSizes);
...
void VulkanEngine::InitSyncStructures()
{
    ...
    for (int i = 0; i < PresentSemaphores.Num(); ++i)
    {
...
void VulkanEngine::DestroySwapchain()
{
    ...
    //destroy resources
    for (int i = 0; i < SwapchainImageViews.Num(); ++i)
    {
